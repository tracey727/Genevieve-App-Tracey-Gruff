...
function showScreen(name, focus = true) {
  ...
  navButtons.forEach(button => button.classList.toggle('is-current', button.dataset.screenTarget === name));
  if (focus) {
...
navButtons.forEach(button => button.addEventListener('click', () => showScreen(button.dataset.screenTarget)));
...
function updateFreshness() {
  const formatter = new Intl.DateTimeFormat('en-AU', { hour: 'numeric', minute: '2-digit', second: '2-digit' });
...
async function checkFoundation() {
  ...
  try {
    ...
    if (data.database === 'connected') status.textContent = 'Neon Stage 1 connected';
    else if (data.database === 'not-configured') status.textContent = 'Structure healthy · DB binding pending';
